...
function Tweet({tweet}:Props) {
   ...
   const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const comment: CommentBody = {
        ...
        comments.tweetId: tweet._id,
        comments.username: session?.user?.name || 'Unknown User',
        comments.profileImg: session?.user?.image || 'https://links.papareact.com/gll',
      }
   ...
   return (
      <div className='flex flex-col space-x-3 border-y p-5 border-gray-100'>
         ...
         {commentBoxVisible && (
            <form onSubmit={handleSubmit} action="" className='mt-3 flex space-x-3'>
               <input
                  value={input}
